...
public class CabinManager {
	...
	public CabinManager(CabinFileHandler cfh) {
		...
		this.cfh = cfh;
		this.locations.addAll(this.cfh.readFromFile());
	}
...
/** Obsolete methods: **/
	 ...
	 *
	private StringBuilder getCabinDetails(Location cab) {
		StringBuilder sb = new StringBuilder();
		Formatter formatter = new Formatter(sb, Locale.UK);
		formatter
	 ...
	 *
	public void printCabDetails(Location cab) {
	 ...
	 *
	public void printDetailsForCabinNumber(String id) {
		try {
			Location cab = this.findLocationById(id);
