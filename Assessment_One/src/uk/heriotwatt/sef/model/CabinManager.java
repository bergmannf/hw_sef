...
public class CabinManager {
	 ...
	 * Therefore the cost for all cabins are added up.
	 * 
	 ...
	 */
	public double getCheapestCabinCost()
	{
		// TODO: Empty array;
	 ...
	 */
	public double getExpensiveCabinCost()
	{
		// TODO: Empty array;
	...
	public void printAllCabins() {
		...
		formatter.format("%1$10s | %2$10s | %3$10s | %4$5s %n", "NUMBER", "OWNER",
				"FACILITIES", "BEDS");
		for (Cabin cab : this.cabins) {
			// TODO Return the initials of the owner.
			formatter
					.format("%1$10d | %2$10s | %3$10s | %4$5d %n", cab
							.getCabinNumber(), cab.getOwner()
							.getInitPeriodLast(), cab.getFacilities()
							.toString().toLowerCase(), cab.getBeds());
		}
	...
	public void printCabDetails(Cabin cab) {
		StringBuilder sb = new StringBuilder();
		Formatter formatter = new Formatter(sb, Locale.UK);
		formatter.format("%1$10s | %2$15s | %3$10s | %4$15s | %5$5s | %6$5s %n", "NUMBER", "OWNER", "FACILITIES", "CONDITION", "BEDS", "ROOMS");
		formatter.format("%1$10d | %2$15s | %3$10s | %4$15s | %5$5d | %6$5d %n",
